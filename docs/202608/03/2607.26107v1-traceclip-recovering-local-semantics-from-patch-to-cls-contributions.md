...
tags: ["query:vvvpg"]
score: 8.0
evidence: 基于CLIP的稠密视觉语言理解，恢复局部语义
tldr: CLIP的图像级全局对齐使局部语义对应仅受间接约束，现有训练-free方法未深入挖掘内部贡献。TraceCLIP通过隔离CLS注意力输出中的patch特定项，恢复patch级语义证据，并利用语义测地拓扑门校准特征。在八个零样本语义分割基准上，mIoU提升1.3-4.5点，超越最强训练-free方法，且无需额外训练或外部模型。表明全局对齐表示内部仍可访问空间局部语义。
source: arxiv
...
motivation: CLIP全局表示缺乏局部语义约束，训练-free方法未利用CLS内部的贡献信息。
method: 从CLS注意力输出中分离patch特定贡献，并用语义测地拓扑门校准最终层patch亲和力。
result: 在8个零样本语义分割基准上，平均mIoU提升1.3-4.5点，超越现有训练-free方法。
conclusion: 证明全局对齐表示内保留可恢复的局部语义，无需额外监督或外部模型。
---

## 摘要
密集视觉-语言理解，包括目标定位、区域识别和开放词汇语义分割，需要将语言概念与空间定位的视觉区域相关联。CLIP通过从大规模对比预训练中学习共享的图像-文本嵌入空间，为这些任务提供了坚实的基础。然而，其图像级目标将文本与CLS派生的全局表示对齐，使得局部视觉-语言对应仅被间接约束。现有方法要么引入额外的监督、外部模型或任务特定的适应，而无训练方法主要从现有的补丁特征中恢复密集响应，而不检查在CLIP内部哪里可以最容易地访问局部语义。我们引入了TraceCLIP，一个无训练框架，通过隔离写入CLS注意力输出的补丁特定术语来恢复潜在的补丁级语义证据。TraceCLIP进一步将源自贡献的语义响应转换为语义测地线拓扑门，该门校准最终层的补丁亲和性以进行密集特征重建。诊断实验表明，这些贡献特征表现出强大的局部语义判别性和文本条件的空间对齐。在八个零样本语义分割基准上，TraceCLIP在平均mIoU上比最强先前无训练方法在两个主干和背景设置中均取得了1.3到4.5个百分点的增益，无需额外训练、外部视觉基础模型或区域级监督。更广泛地说，这些发现表明，空间定位的语义可能仍可在全局对齐表示的内部构造中访问。
